...
async function whichCommand(cmd: string): Promise<{ found: boolean; path?: string; version?: string }> {
  try {
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        return toolError(`Failed to check dependencies: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ file_path }) => {
      try {
        ...
        return toolError(`Failed to get media info: ${(error as Error).message}`);
      }
    },
  );
    ...
    async ({ file_path, interval_seconds, max_frames, format }) => {
      ...
      try {
        ...
        const actualInterval =
          totalPossibleFrames > max_frames
        ...
        const frames: Array<{ base64: string; mimeType: string; timestamp: number }> = [];
        ...
        return toolResultWithImages(
          metadata,
          frames.map((f) => ({ base64: f.base64, mimeType: f.mimeType })),
        );
      } catch (error) {
        return toolError(`Failed to extract frames: ${(error as Error).message}`);
      } finally {
  ...
  server.registerTool(
    ...
    async ({ file_path, output_dir }) => {
      ...
      try {
        ...
        await execCommand(
          "ffmpeg",
          ["-i", file_path, "-vn", "-acodec", "libmp3lame", "-q:a", "5", "-y", audioPath],
          { timeout: 120_000 },
        );
        ...
        return toolError(`Failed to extract audio: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ file_path, chunk_minutes, output_dir }) => {
      ...
      try {
        ...
        if (tempDir) await cleanupTempDir(tempDir);
        return toolError(`Failed to split audio: ${(error as Error).message}`);
      }
    },
  );
}
